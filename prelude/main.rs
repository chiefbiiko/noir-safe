...
async fn main() {
    ...
    let mut ap_prover_file = std::fs::File::create(format!("{}/../circuits/account_proof/Prover.toml", cargo_manifest_dir))
        .expect("ap_prover_file");
    ap_prover_file.write_all(prover_toml.as_bytes())
        .expect("ap_prover_file write");
    let mut sp_prover_file = std::fs::File::create(format!("{}/../circuits/storage_proof/Prover.toml", cargo_manifest_dir))
