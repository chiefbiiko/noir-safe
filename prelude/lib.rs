...
pub async fn fetch_inputs(
    ...
    Ok((
        latest.as_u64(),
        Inputs {
            safe_address: u8_to_u64_arr(safe_address.into()),
            msg_hash: u8_to_u64_arr(msg_hash.into()),
            header_rlp: rlp_encode_header(&block),
            state_root: u8_to_u64_arr(block.state_root.into()),
            storage_root: u8_to_u64_arr(proof.storage_hash.into()),
            state_trie_key: u8_to_u64_arr(keccak256(&safe_address)),
            storage_trie_key: u8_to_u64_arr(keccak256(&storage_key)),
